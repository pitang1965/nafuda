...
async function generateUniqueUrlId(): Promise<string> {
  while (true) {
    ...
    const candidate = Array.from(bytes).map(b => URLID_CHARS[b % 36]).join('')
...
export const createPersona = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const existingUrlId = await db.select({ urlId: urlIds.urlId })
      .from(urlIds)
      .where(eq(urlIds.userId, session.user.id))
      .limit(1)
    if (!existingUrlId[0]) {
      const newUrlId = await generateUniqueUrlId()
      await db.insert(urlIds).values({ urlId: newUrlId, userId: session.user.id })
    }
