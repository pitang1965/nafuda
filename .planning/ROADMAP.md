...
- [x] **Phase 2: イベント・チェックイン** - イベントチェックインと同じ現場にいた人一覧を実装する (completed 2026-04-26)
- [x] **Phase 3: QR・コネクション・PWA** - QRコード表示・コネクション記録・PWAオフライン対応を完成させる (completed 2026-04-27)
...
- [ ] 01-05-PLAN.md — UAT gap closure: ログインリンク修正・ログアウトボタン・ウィザードナビゲーション・loaderキャッシュ無効化

### Phase 2: イベント・チェックイン
**Goal**: 主催者がイベントを作成して QR を共有し、参加者が QR スキャン → 参加ボタンでチェックインでき、同じイベントの参加者一覧（同担フィルタ適用済み）を閲覧できる。未ログインユーザーはハンドル名・アバターのみ閲覧できる
**Depends on**: Phase 1
**Requirements**: OSHI-03, OSHI-04, OSHI-05
**Success Criteria** (what must be TRUE):
  1. ログインユーザーはイベント名・会場名・日付を入力してイベントを作成でき、/e/$slug にリダイレクトされる
  2. /e/$slug ページに QRコード（そのページの URL）が表示され、「参加する」ボタンをクリックするとチェックインが記録される
  3. ログインユーザーは参加者一覧を閲覧でき、同担拒否フラグを設定したユーザーは一覧に表示されない
  4. 未ログインユーザーがイベントページにアクセスすると参加者のハンドル名とアバターのみが表示され、個別プロフィールのリンクはクリックできない
**Plans**: 4 plans

Plans:
- [ ] 02-01-PLAN.md — events テーブルに hostUserId カラム追加マイグレーション + サーバー関数改修（createEvent・getMyEvents追加、checkinToEvent・getEventParticipants改修、旧関数削除）
- [ ] 02-02-PLAN.md — /events 一覧ページ（主催・参加の2セクション）+ /events/new 作成フォーム
- [ ] 02-03-PLAN.md — /e/$slug にQRコード・「参加する」ボタン追加 + 旧コンポーネント削除
- [ ] 02-04-PLAN.md — Phase 2 エンドツーエンド動作確認（checkpoint:human-verify）
...
| 3. QR・コネクション・PWA | 5/5 | Complete   | 2026-04-27 |
